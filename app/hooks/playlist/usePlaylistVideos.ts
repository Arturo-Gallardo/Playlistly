...
import {
  ...
  toPlaylistVideoWire,
} from "../../lib/playlist/playlist-video";
import { getPlaylistVideoCacheKey } from "../../lib/playlist/playlist-cache";
import type {
  PlaylistApiError,
  PlaylistApiSuccess,
  PlaylistVideo,
...
function getCachedPlaylistVideos(playlist: string) {
  try {
    const cachedValue = window.localStorage.getItem(getPlaylistVideoCacheKey(playlist));
    ...
    if (!isPlaylistVideoCacheEntry(cachedData)) {
      ...
      if (Date.now() - legacyVideos.savedAt > playlistCacheTtlMs) {
        window.localStorage.removeItem(getPlaylistVideoCacheKey(playlist));
        return null;
      }
...
function setCachedPlaylistVideos(playlist: string, videos: PlaylistVideo[]) {
  ...
  try {
    window.localStorage.setItem(
      getPlaylistVideoCacheKey(playlist),
