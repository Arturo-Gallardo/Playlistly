...
import { getVideoDisplayDetails } from "../../../lib/playlist/playlist-video";
...
export function CanvasVideoDetailsPanel({ details }: CanvasVideoDetailsPanelProps) {
  const displayDetails = getVideoDisplayDetails(details.video);
  ...
  return (
    <aside className="pointer-events-none fixed bottom-5 left-5 z-20 flex max-w-2xl gap-4 rounded-md border border-white/15 bg-[#111111]/45 p-3 shadow-2xl backdrop-blur-sm">
      ...
      <div className="min-w-0 self-center pr-10">
        ...
        {displayDetails.publishedLabel ? (
          <p className="font-control mt-2 text-[10px] uppercase tracking-[0.2em] text-white/45">
            published {displayDetails.publishedLabel}
          </p>
        ) : null}
      </div>
    </aside>
